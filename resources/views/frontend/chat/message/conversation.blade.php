@extends('frontend.layouts.appchat')
@section('title', __('ChatAsUs'))
                ...
                @if($users->count())
                    @foreach($users as $user)
                        <li class="chat-user-list
                            ...
                            <a href="{{ route('frontend.message.conversation', $user->id) }}">
            ...
            <ul class="list-group list-chat-item">
                @if($groups->count())
                    @foreach($groups as $group)
                        <li class="chat-user-list">
                            <a href="{{ route('frontend.message-groups.show', $group->id) }}">
                            {{ $group->name }}
                            </a>
                        </li>
                    @endforeach
...
<div class="modal" tabindex="-1" role="dialog" id="addGroupModal">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            ...
            <form action="{{ route('frontend.message-groups.store') }}" method="post">
                {{ csrf_field() }}
...
<script>
    $(function (){
        ...
        let ip_address = '192.168.10.10';
        let socket_port = '8005';
        let socket = io(ip_address + ':' + socket_port, {transports: ['websocket', 'polling']});
        ...
        function sendMessage(message) {
            let url = "{{ route('frontend.message.send-message') }}";
            let form = $(this);
...
{{-- <div class="row chat-row">
        ...
        <div class="col-md-9 chat-section">
            ...
            <div class="chat-body" id="chatboard">
                <div class="message-listing" id="messageWrapper">
...
$(function (){
            ...
            socket.on("channel:App\\Events\\PrivateMessageEvent", function (message)
            {
               appendMessageToReceiver(message);
            });   
            });
